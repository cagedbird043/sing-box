...
func (s *ProviderRemote) StartContext(ctx context.Context, startContext *adapter.HTTPStartContext) error {
	...
	s.httpClient, err = s.resolveHTTPClient(startContext)
	if err != nil {
...
func (s *ProviderRemote) resolveHTTPClient(startContext *adapter.HTTPStartContext) (*http.Client, error) {
	var dialer N.Dialer
	if s.downloadDetour != "" {
		...
		dialer = boxDialer.NewDefaultOutbound(s.ctx)
	}
	if startContext != nil {
		return startContext.HTTPClient(s.downloadDetour, dialer), nil
	}
