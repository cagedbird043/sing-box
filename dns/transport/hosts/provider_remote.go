...
func (p *RemoteProvider) resolveTransport() (adapter.HTTPTransport, error) {
	...
	if p.httpClientOptions != nil && !p.httpClientOptions.IsEmpty() {
		return httpClientManager.ResolveTransport(p.ctx, p.logger, *p.httpClientOptions)
	}
