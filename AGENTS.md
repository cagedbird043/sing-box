...
5. Verify submodule pointers match the upstream tag. `git rebase` does not update gitlinks:
   ...
   # Must be identical. Restore the release-tag gitlinks only in a clean candidate.
   ```
...
The `clients/apple` gitlink must also remain identical to the selected release
tag even though the cagedbird workflow currently publishes Android artifacts.
