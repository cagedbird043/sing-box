...
the core tag points at Android `aed2b6ea` (`Bump version 1.13.19`), which fails
both the libbox minSdk contract and Kotlin API compilation. The pinned official
Android history revision `1877f716` (`Bump version 1.14.0-beta.17`) passes the
complete `:app:assembleOtherRelease` gate with the beta.17 core.
